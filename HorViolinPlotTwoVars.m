function [Handles_all] = HorViolinPlotTwoVars(Y,X,Group,ColZ)
Kernel_bandwidth = 0.01; % in percentage of range
if nargin < 2
    X = Y;
    Y = 1;
end
...
for i_col = 1:size(X,2)
   ...
   tot_X    = sum(ind_ok); 
   i_Y      = Y(ind_ok,i_col);
   BandWidth = Kernel_bandwidth.*(range(i_X));
   for i_gr = 1:num_gr
       ind_iX_iGr = Group(ind_ok,i_col) == wh_gr(i_gr);
       num_indIXGR = sum(ind_iX_iGr);
       if num_indIXGR>0
           [f,xi] = ksdensity(i_X(ind_iX_iGr),'bandwidth',BandWidth);
           [f_all,xi_all] = ksdensity(i_X,'bandwidth',BandWidth);
           xi = [xi(1) xi xi(end)];
           f = [0 f 0];
       ...
       %iPDF = f;
       iPDF = 0.45.*(f./max(f_all(:)));
       for_plot_X = [xi fliplr(xi)];
       if i_gr == 1
           for_plot_Y = Y(1)+[iPDF 0.*fliplr(-iPDF)];
           scatter_y = Y(1)+abs(scatter_y);
       elseif i_gr == 2
           for_plot_Y = Y(1)+[0.*iPDF fliplr(-iPDF)];
           scatter_y = Y(1)-abs(scatter_y);
       end
